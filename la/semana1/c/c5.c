...
#define MAXN 10000 // Define o número máximo de entradas que o programa pode manipular
...
int main() { // Início da função principal
    ...
    for(int j = 0; j < n; j++) { // Loop para processar cada entrada
        if(1 <= e[j] && e[j] <=5 && m[j] <= 7 && 1 <= m[j]) { // Verifica se 'e' e 'm' estão dentro dos limites especificados
            ...
            int expoente   = 0; // Inicializa a variável que armazena o expoente
            for(int i = e[j]+1; i >= 1; i--) { // Loop para calcular o expoente
                if(bin[j][i] == '1')  { expoente   = expoente   + pow(2, e[j]-i); } // Se o caractere na posição 'i' é '1', adiciona 2^(e[j]-i) ao expoente
            }
            ...
            if(allOne) { // Se todos os caracteres são '1'
                ...
                results[j] = pow(-1, bin[j][0]) * (1+man) * pow(2, expoente - excesso); // Calcula o resultado e define o tipo como "number"
                strcpy(types[j], "number");
            }
        }
    }
